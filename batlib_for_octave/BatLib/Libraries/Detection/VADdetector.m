function [D,LR,PR,Px,iPx,Py,iPy,Vn,logl] = VADdetector(Y,VAD,NE)
...
%   Home page: http://www.ee.ic.ac.uk/hp/staff/dmb/voicebox/voicebox.html
 ...
 % default algorithm constants
    ...
    if nargin < 2 || isempty(VAD)
    vad.dt  = 5e-3;     % true frame increment time
    vad.pr  = 0.7;      % Speech probability threshold
    vad.gx  = 1000;     % maximum posterior SNR = 30dB
    vad.gz  = 1e-4;     % minimum posterior SNR = -40dB
    
    vad.ne  = 0;        % noise estimation: 0=min statistics, 1=MMSE [0]
    
    vad.ge  = 1;        % xi estimation: 0= Itakura Saito (ISD), 1 = Decision Directed (DD) [1]
    vad.ta  = 0.396;    % Time const for smoothing SNR estimate = -tinc/log(0.98) from [2]
    vad.xn  = 0;        % minimum prior SNR = -Inf dB
    
    vad.hmm = 0;        % HMM-Based Hang Over [0]
    vad.ts  = 0.1;      % mean talkspurt length (100 ms)
    vad.tn  = 0.05;     % mean silence length (50 ms)
    else
        vad = VAD;
    end
...
%     b10=a11-a10*a01/a00;
    ...
    [I,J] = size(Y);
        ...
        switch vad.ne
            case 1
            [dp]=estnoiseg(Y',dt,ne);	% estimate the noise using MMSE
            case 0
            [dp]=estnoisem(Y',dt,ne);	% estimate the noise using minimum statistics
        end
        ...
        gamma   = max(min(Y./dp',gmax),gz);  % gamma = posterior SNR [2](10)
        logl    = zeros(I,J);
        logL    = zeros(1,J);
...
%       
        ...
        if vad.ne == 0;
            if ~isempty(Onset) && ~isempty(ne)
                if Onset(1) < round(ne.td/vad.dt)
        ...
        P = length(Onset);
        iPmax = zeros(P,1,'uint32');
        Pmax  = nan(P,1);
        
        for p = 1:P;
            [pmax,ipmax] = max(LR(Onset(p):Offset(p)));
